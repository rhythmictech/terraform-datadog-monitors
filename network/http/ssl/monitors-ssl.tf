...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:network", "provider:http_check", "resource:ssl-certificate", "created-by:terraform"], var.invalid_ssl_certificate_extra_tags)
}
...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:network", "provider:http_check", "resource:ssl-certificate", "created-by:terraform"], var.certificate_expiration_date_extra_tags)
}