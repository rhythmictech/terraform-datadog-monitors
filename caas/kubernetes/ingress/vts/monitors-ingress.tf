...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:caas", "provider:prometheus", "resource:nginx-ingress-controller", "created-by:terraform"], var.ingress_5xx_extra_tags)
}
...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:caas", "provider:prometheus", "resource:nginx-ingress-controller", "created-by:terraform"], var.ingress_4xx_extra_tags)
}
