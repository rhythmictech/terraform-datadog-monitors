...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:middleware", "provider:kong", "resource:kong", "created-by:terraform"], var.not_responding_extra_tags)
}
...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:middleware", "provider:kong", "resource:kong", "created-by:terraform"], var.treatment_limit_extra_tags)
}
