...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:middleware", "provider:php-fpm", "resource:php-fpm", "created-by:terraform"], var.php_fpm_connect_extra_tags)
}
...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:middleware", "provider:php-fpm", "resource:php-fpm", "created-by:terraform"], var.php_fpm_busy_extra_tags)
}
