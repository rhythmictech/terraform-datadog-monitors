resource "datadog_monitor" "keyvault_status" {
  ...
  query = <<EOQ
      ${var.status_time_aggregator}(${var.status_timeframe}): (
        avg:azure.keyvault_vaults.count${module.filter-tags-status.query_alert} by {resource_group,region,name}
      ) < 1
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:azure", "resource:keyvault", "created-by:terraform"], var.status_extra_tags)
}
...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:azure", "resource:keyvault", "created-by:terraform"], var.api_result_extra_tags)
}
...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:azure", "resource:keyvault", "created-by:terraform"], var.api_latency_extra_tags)
}
