...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:azure", "resource:azure-search", "created-by:terraform"], var.latency_extra_tags)
}
...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:azure", "resource:azure-search", "created-by:terraform"], var.throttled_queries_rate_extra_tags)
}
