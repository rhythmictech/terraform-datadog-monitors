resource "datadog_monitor" "apimgt_status" {
  ...
  query = <<EOQ
      ${var.status_time_aggregator}(${var.status_timeframe}):avg:azure.apimanagement_service.count${module.filter-tags-status.query_alert} by {resource_group,region,name} < 1
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:azure", "resource:apimanagement", "created-by:terraform"], var.status_extra_tags)
}
...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:azure", "resource:apimanagement", "created-by:terraform"], var.failed_requests_extra_tags)
}
...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:azure", "resource:apimanagement", "created-by:terraform"], var.other_requests_extra_tags)
}
...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:azure", "resource:apimanagement", "created-by:terraform"], var.unauthorized_requests_extra_tags)
}
...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:azure", "resource:apimanagement", "created-by:terraform"], var.successful_requests_extra_tags)
}
