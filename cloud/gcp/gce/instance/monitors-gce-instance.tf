...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:gcp", "resource:gce-instance", "created-by:terraform"], var.cpu_utilization_extra_tags)
}
...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:gcp", "resource:gce-instance", "created-by:terraform"], var.disk_throttled_bps_extra_tags)
}
...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:gcp", "resource:gce-instance", "created-by:terraform"], var.disk_throttled_ops_extra_tags)
}
