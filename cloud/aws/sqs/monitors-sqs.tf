...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:aws", "resource:sqs", "created-by:terraform"], var.visible_messages_extra_tags)
}
...
EOQ
  ...
  tags = concat(["env:${var.environment}", "type:cloud", "provider:aws", "resource:sqs", "created-by:terraform"], var.age_of_oldest_message_extra_tags)
}
